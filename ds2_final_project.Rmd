...
train_price_tree = sample(1:nrow(airbnb_data), nrow(airbnb_data)/2)

# grow tree 
fit_price = rpart(price ~ ., data = airbnb_data, subset = train_price_tree)
...
pred_fit_price = predict(fit_price, newdata = airbnb_data[-train_price_tree,])


# no pruning
ctree_price = ctree(price~., airbnb_data, subset = train_price_tree)

#summary(ctree.boston)
#plot(ctree_price) useless

pred_ctree_price = predict(ctree_price, newdata = airbnb_data[-train_price_tree,])


# tune over maximum depth, method = "rpart2" (plots Max Tree depth)
# tune over cp, method = "rpart" (plots Complexity Parameter)
#rpartTune = train(airbnb_data[,-11], airbnb$price, 
#                   method = "rpart2",
...
basic.mse
```
the optimal tree size equals to 3, the training data error rate is 0.4531. Use test dataset to predict, the error rate is 0.4472914.  
...
#0.4257287 is the test error rate
```
